...
int8_t switch_1_timer = -1;
char switch_2_status = 0;
int8_t switch_2_timer = -1;
...
static void switch_1_off(){
	pinMode(SWITCH_1_PIN, INPUT);
	switch_1_status=0;
	switch_1_timer=-1;
}
static void switch_2_off(){
	pinMode(SWITCH_2_PIN, INPUT);
	switch_2_status = 0;
	switch_2_timer=-1;
}
...
void localReceive_process(RF24Packet* newPacket, size_t packetSize){
	...
	if(newPacket->type == 1){// command packet
		...
		if(newPacket->content[4] == 1){// target switch 1
			...
			if(newPacket->content[5] == 3){// turn on
				pinMode(SWITCH_1_PIN, OUTPUT);
				switch_1_status = 1;
				if(switch_1_timer!=-1) loop_timer->stop(switch_1_timer);
				int time_on = 3000;
				if(packetSize==19) time_on = big_to_int(newPacket->content+6)*5;
				switch_1_timer = loop_timer->after(time_on, switch_1_off);
			}else return;
			...
			if(newPacket->content[5] == 3){// turn on
				...
				switch_2_status = 1;
				if(switch_2_timer!=-1) loop_timer->stop(switch_2_timer);
				int time_on = 3000;
				if(packetSize==19) time_on = big_to_int(newPacket->content+6)*5;
				switch_2_timer = loop_timer->after(time_on, switch_2_off);
			}else return;
		}else return;
		RF24Packet reply;
		reply.type = 2;
